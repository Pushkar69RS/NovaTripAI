...
Stack: Python 3.12 (uv), FastAPI, uvicorn, pydantic + pydantic-settings, jinja2, httpx, numpy; ruff + pytest.
Planned services: Supabase Postgres + pgvector, OpenRouter (deepseek/deepseek-chat), local multilingual-e5-small embeddings.
Exists now: uv project + git, app/main.py with GET /health, tests/test_health.py, CLAUDE.md, docs/, .env.example, .gitignore.
